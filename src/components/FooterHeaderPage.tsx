...
interface FooterHeaderPageProps extends Pick<FooterHeaderProps, "render"> {
  register:
    | PDFContextValues["registerFooter"]
    | PDFContextValues["registerHeader"];
  pages: number;
}

export const FooterHeaderPage = ({
  pages,
  register,
  render,
}: FooterHeaderPageProps) => {
  const children = Array(pages)
    .fill(null)
    .map((_, index) => {
      return (
        <div
  ...
  return <PreviewPortal>{children}</PreviewPortal>;
};
