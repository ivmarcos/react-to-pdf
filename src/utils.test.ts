...
describe('utils', () => {
  ...
  describe('cropY', () => {
    test('should work', () => {
      ...
      expect({width: croppedCanvas.width, height: croppedCanvas.height}).toEqual({
        width: 400,
        height: 700
      })
    })
  ...
  describe('calculateHeightOffset', () => {
    test('return the height when is less than the max height', () => {
      expect(utils.calculateHeightOffset({maxHeight: 1000,height: 999, offsetY: 300})).toBe(999)
    })
    test('return the offset height when is less than the max height', () => {
      expect(utils.calculateHeightOffset({maxHeight: 1000,height: 1200, offsetY: 300})).toBe(900)
    })
    test('return the max height when is higher than the offset height', () => {
      expect(utils.calculateHeightOffset({maxHeight: 1000,height: 3000, offsetY: 300})).toBe(1000)
    })
  })

  describe('calculateFitRatio', () => {
    test('return the correct ratio when the size is greater than the max size', () => {
      expect(utils.calculateFitRatio({maxSize: 5, size: 10})).toBe(.5)
    })
  ...
  describe('calculateFillRatio', () => {
    ...
    test('return the correct ratio when the size is higher or equal than the target size ',() => {
      expect(utils.calculateFillRatio({targetSize: 10, size: 20})).toBe(.5)
    })
  })
